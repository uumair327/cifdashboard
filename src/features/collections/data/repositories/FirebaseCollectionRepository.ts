  ...
  implements ICollectionRepository<T>
{
  ...
  async getAll(): Promise<T[]> {
    try {
      console.log(`[FirebaseRepo] Fetching all from collection: ${this.collectionName}`);
      const collectionRef = collection(db, this.collectionName);
      const snapshot = await getDocs(collectionRef);
      console.log(`[FirebaseRepo] Found ${snapshot.docs.length} documents in ${this.collectionName}`);
      
      const results = snapshot.docs.map(doc => this.docToEntity(doc.id, doc.data()));
      console.log(`[FirebaseRepo] Mapped results:`, results);
      return results;
    } catch (error) {
